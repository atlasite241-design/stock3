...
function Content() {
  ...
  const prodMap = new Map<string, { name: string; category: string; qty: number; revenue: number; lastSold: string | null }>()
  products.forEach((p) => prodMap.set(p.id, { name: p.name, category: p.category, qty: 0, revenue: 0, lastSold: null }))
  sales.forEach((s) =>
    s.items.forEach((i) => {
      ...
      if (!e.lastSold || s.date > e.lastSold) e.lastSold = s.date
    })
  )
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{t('rpwp_title')}</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-zinc-400">{t('rpwp_subtitle')}</p>
      </motion.div>
          ...
          <table className="w-full min-w-[680px]">
            <thead>
              <tr className="border-b border-gray-100 dark:border-white/10 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400 dark:text-zinc-500">
                ...
                <th className="px-5 py-3.5">{t('rpwp_col_last_sold')}</th>
              </tr>
            </thead>
            <tbody>
              {ranked.map((p, i) => (
                <tr key={p.name} className="border-b border-gray-50 dark:border-white/5 transition-colors hover:bg-amber-50/40 dark:hover:bg-white/5">
                  <td className="px-5 py-3.5">
                    <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-500/10 text-xs font-bold text-rose-600 dark:text-rose-400">
                      {i + 1}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-sm text-gray-500 dark:text-zinc-400">
                    {p.lastSold ? new Date(p.lastSold).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function RapportWorstProduitsPage() {
  return (
    <AppShell>
      <Content />
    </AppShell>
