...
export default function MobileParametresPage() {
  ...
  return (
    <MobileSubShell title={t('mob_set_title')} subtitle={t('mob_set_subtitle')}>
          ...
          <div className="space-y-1.5">
            {stores.map((s) => (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${s.id === activeStoreId ? 'bg-sky-500/15' : 'hover:bg-slate-100 dark:bg-white/5'}`}
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-200 dark:bg-white/10 text-sky-300"><Building2 className="h-4 w-4" /></span>
      ...
      <Link
        href="/"
        className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 dark:border-sky-500/20 bg-slate-100 dark:bg-white/5 py-3.5 text-sm font-semibold text-sky-300 transition active:scale-[0.98]"
      >
