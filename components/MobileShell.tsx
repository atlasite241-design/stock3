...
export default function MobileShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className="relative mx-auto min-h-screen w-full max-w-md overflow-x-hidden bg-slate-100 dark:bg-[#0b1326] pb-32 text-slate-700 dark:text-slate-200">
      ...
      {/* Top app bar */}
      <header className="fixed left-1/2 top-0 z-50 flex w-full max-w-md -translate-x-1/2 items-center justify-between gap-2 border-b border-slate-200 dark:border-amber-500/20 bg-white/80 dark:bg-slate-950/60 px-5 py-3 backdrop-blur-2xl">
        <Link href="/mobile" className="flex min-w-0 items-center gap-2.5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-amber-500/40 bg-gradient-to-br from-amber-400 to-yellow-500 text-xs font-black text-slate-900">
            {initials}
          </div>
          <span className="truncate text-lg font-black italic tracking-tight text-amber-400">DrogueriePro</span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {activeStore && (
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-1.5 text-xs font-bold text-amber-300 ${canSwitch ? '' : 'cursor-default'}`}
              >
              ...
              {storeOpen && canSwitch && (
                <>
                  ...
                  <div className="absolute right-0 top-11 z-50 w-60 overflow-hidden rounded-2xl border border-slate-200 dark:border-amber-500/20 bg-white dark:bg-slate-900 p-1.5 shadow-2xl">
                    {allowedStores.map((s) => (
                      <button
                        key={s.id}
                        onClick={() => (s.id === activeStoreId ? setStoreOpen(false) : switchStore(s.id))}
                        className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${s.id === activeStoreId ? 'bg-amber-500/10' : 'hover:bg-slate-100 dark:bg-white/5'}`}
                      >
                        <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${s.active ? 'bg-amber-500/15 text-amber-300' : 'bg-slate-200 dark:bg-white/10 text-slate-500 dark:text-slate-400'}`}>
          ...
          <Link href="/mobile/plus/notifications" className="relative flex h-9 w-9 items-center justify-center rounded-full text-amber-400 transition-colors hover:bg-amber-500/10">
