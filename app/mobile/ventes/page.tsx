  ...
  return (
    <>
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-bold text-white">{t('mob_sales_title')}</h1>
        <p className="text-sm text-slate-400">{t('mob_sales_subtitle')}</p>
      </motion.section>

      {/* Day totals */}
      <section className="grid grid-cols-2 gap-4">
        <div className="rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-5 backdrop-blur-xl">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-sky-400/80">{t('mob_total_day')}</p>
          <p className="mt-1 text-xl font-bold text-white tabular-nums">{fmtDH(model.total)}</p>
        </div>
        <div className="rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-5 backdrop-blur-xl">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-sky-400/80">{t('mob_count_day')}</p>
          <p className="mt-1 text-xl font-bold text-white tabular-nums">{model.count}</p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-5 backdrop-blur-xl">
        <div className="grid grid-cols-2 gap-3">
          {modes.map((m) => (
            <div key={m} className="flex items-center justify-between rounded-xl bg-white/5 px-3 py-2.5">
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${PAY_CHIP[m]}`}>{PAYMENT_META[m].label}</span>
              <span className="text-sm font-semibold text-white tabular-nums">{fmtDH(model.byMode[m])}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-white">{t('mob_recent')}</h3>
        ...
        <div className="space-y-3">
          {model.recent.map((s) => (
            <div key={s.id} className="flex items-center gap-3 rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-4 backdrop-blur-xl">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-white tabular-nums">{fmtDH(s.total)}</p>
          ...
          {model.recent.length === 0 && (
            <p className="rounded-2xl border border-sky-500/20 bg-sky-500/[0.06] p-6 text-center text-sm text-slate-400">{t('mob_no_sales')}</p>
          )}
        </div>
      </section>
    </>
