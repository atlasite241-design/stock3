...
const LABEL: Record<string, string> = {
  entree: 'Entrée', reappro: 'Réappro', sortie: 'Sortie', ajustement: 'Ajustement',
  vente: 'Vente', reception: 'Réception', retour: 'Retour', inventaire: 'Inventaire',
  transfert_out: 'Transfert sortant', transfert_in: 'Transfert entrant', stock_initial: 'Stock initial',
}
...
function Content() {
  ...
  const cancel = () => {
    ...
    toast(`✓ Mouvement annulé : ${target.productName} (${target.qty > 0 ? '+' : ''}${target.qty})`)
    setTarget(null)
  }
    ...
    <>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">
          <Undo2 className="h-6 w-6 text-amber-500" />Annulation de mouvement
        </h1>
        <p className="mt-1 max-w-3xl text-sm text-gray-500 dark:text-zinc-400">
          Neutralise un mouvement en enregistrant son inverse — l’historique reste intact —{' '}
      ...
      <div className="glass-card p-3">
        <div className="relative">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)}
            placeholder="Rechercher un produit ou une note…" className="input-field pl-9" />
        </div>
      </div>
      ...
      <div className="glass-card overflow-x-auto">
        {rows.length === 0 ? (
          <div className="flex flex-col items-center gap-3 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-zinc-400">Aucun mouvement à afficher.</p>
          </div>
        ) : (
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:border-white/10 dark:text-zinc-500">
                ...
                <th className="px-4 py-3">Type</th><th className="px-4 py-3 text-center">Quantité</th>
                <th className="px-4 py-3">Note</th><th className="px-4 py-3 text-right" />
              </tr>
            </thead>
            <tbody>
              {rows.map((m) => {
                ...
                return (
                  <tr key={m.id} className="border-b border-gray-50 last:border-0 dark:border-white/5">
                    ...
                    <td className="px-4 py-2.5 text-xs text-gray-500">{LABEL[m.type] ?? m.type}</td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {isCancelled ? (
                        <span className="rounded-md bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-500 dark:bg-white/10">déjà annulé</span>
                      ) : isCancellation ? (
                        <span className="rounded-md bg-violet-50 px-2 py-0.5 text-[10px] font-bold text-violet-600 dark:bg-violet-500/10 dark:text-violet-400">annulation</span>
                      ) : (
      ...
      <DangerConfirm
        ...
        description={
          target ? (
            <>
              Un mouvement inverse de{' '}
              <span className="font-bold text-gray-900 dark:text-white">{-target.qty > 0 ? '+' : ''}{-target.qty}</span>{' '}
              sera enregistré sur <span className="font-bold text-gray-900 dark:text-white">{target.productName}</span>.
              L’historique d’origine est conservé.
            </>
          ) : ''
        }
        word="ANNULER"
        actionLabel="Annuler le mouvement"
      />
    </>
  )
}
