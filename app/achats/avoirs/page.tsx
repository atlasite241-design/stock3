...
function Content() {
  const { ready, creditNotes, validateCreditNote, cancelCreditNote, consumeCreditNote, annulerUtilisationAvoir } = useDroguerie()
                  ...
                  <tr key={a.id} className="border-b border-gray-50 last:border-0 dark:border-white/5">
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex justify-end gap-1">
                        ...
                        <button
                          onClick={() => valider(a)}
                          disabled={!((a.status === 'brouillon' || a.status === 'controle') && can('purch.credit_note'))}
                        ...
                        <button
                          ...
                          disabled={!((a.status === 'valide' || a.status === 'partiel') && reste > 0 && can('purch.credit_note'))}
                        ...
                        <button
                          ...
                          disabled={!(a.status !== 'annule' && a.status !== 'solde' && creditNoteUsed(a) === 0 && can('purch.credit_note'))}
                ...
                {docTarget.uses.map((u) => (
                  <div key={u.id} className="flex items-center justify-between border-b border-gray-50 py-1.5 last:border-0 dark:border-white/5">
                    ...
                    <span className="flex items-center gap-2 font-semibold tabular-nums text-gray-900 dark:text-white">
                      {fmtDH(u.amount)}
                      {u.refund && can('purch.credit_note') && (
                        <button
                          onClick={() => {
