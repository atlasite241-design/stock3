...
import { Check, CreditCard, Lock, Minus, Plus, Search, ShoppingCart, Trash2, Wallet } from 'lucide-react'
...
function Content() {
  ...
  if (!currentSession) {
    return (
      <section className="mt-10 rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-6 backdrop-blur-xl">
        <div className="flex flex-col items-center gap-3 text-center">
          <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-300">
            <Lock className="h-7 w-7" />
          </span>
          <h2 className="text-lg font-bold text-white">{t('mob_pos_closed_title')}</h2>
          <p className="text-sm text-slate-400">{t('mob_pos_closed_desc')}</p>
        </div>
  ...
  if (lastSale) {
    return (
      <motion.section initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="mt-10 rounded-2xl border border-emerald-500/30 bg-emerald-500/[0.08] p-8 text-center backdrop-blur-xl">
        ...
        <h2 className="mt-4 text-lg font-bold text-white">{t('mob_pos_success')}</h2>
  ...
  return (
    <>
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-bold text-white">{t('mob_pos_title')}</h1>
        <p className="text-sm text-slate-400">{t('mob_pos_subtitle')}</p>
      </motion.section>
      ...
      <section className="grid grid-cols-2 gap-3">
        {matches.map((p) => (
          <button
            ...
            className="flex flex-col overflow-hidden rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] text-left backdrop-blur-xl transition active:scale-[0.97]"
          >
            <div className="relative aspect-[5/3] w-full overflow-hidden bg-slate-900/60">
      ...
      <section className="rounded-2xl border border-sky-500/20 bg-sky-500/[0.08] p-4 backdrop-blur-xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-base font-semibold text-white"><ShoppingCart className="h-4 w-4 text-sky-400" />{t('mob_pos_cart')}</h3>
          {cart.length > 0 && <button onClick={() => setCart([])} className="text-xs font-semibold text-rose-400">✕</button>}
        </div>
        {cart.length === 0 ? (
          <p className="py-4 text-center text-sm text-slate-400">{t('mob_pos_empty')}</p>
        ) : (
            ...
            {cart.map((i) => (
              <div key={i.productId} className="flex items-center gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400 tabular-nums">{fmtDH(i.price)} × {i.qty}</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => changeQty(i.productId, -1)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-white"><Minus className="h-3.5 w-3.5" /></button>
                  <span className="w-6 text-center text-sm font-bold text-white tabular-nums">{i.qty}</span>
                  <button onClick={() => changeQty(i.productId, 1)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-white"><Plus className="h-3.5 w-3.5" /></button>
      ...
      <section className="space-y-4">
        <div>
          ...
          <select
            ...
            className="h-11 w-full rounded-2xl border border-sky-500/20 bg-white/5 px-3 text-sm text-white outline-none focus:border-sky-400/60"
          >
            <option value="" className="bg-slate-900">{t('mob_pos_client_none')}</option>
            {clients.map((c) => (
            ...
            {PAY_OPTS.map((o) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 rounded-2xl border py-3 text-xs font-semibold transition ${
                  pay === o.key ? 'border-sky-400/60 bg-sky-500/15 text-sky-300' : 'border-sky-500/20 bg-white/5 text-slate-400'
                }`}
              >
                {o.icon}
                {o.label}
