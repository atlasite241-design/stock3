...
function Content() {
  ...
  const save = () => {
    saveSettings({ ...form, currency: form.currency.trim() || 'MAD (DH)' })
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{t('psub_currency_title')}</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-zinc-400">{t('psub_currency_subtitle')}</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.4 }} className="glass-card max-w-xl p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white">{t('psub_currency_title')}</h2>
            <p className="text-xs text-gray-500 dark:text-zinc-400">{t('psub_currency_subtitle')}</p>
          </div>
        </div>
...
export default function ParametresDevisePage() {
  return (
    <AppShell>
      <Content />
    </AppShell>
